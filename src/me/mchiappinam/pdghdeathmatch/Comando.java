...
public class Comando implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(cmd.getName().equalsIgnoreCase("cf")) {
			...
			((Player)sender).getInventory().setContents(plugin.inventario);
			((Player)sender).getInventory().setArmorContents(plugin.armadura);
			/*if (args.length==0) {
			...
			if(args[0].equalsIgnoreCase("admin")) {
				...
				if(args[1].equalsIgnoreCase("setpos")) {
					...
					if(args.length>2) {
						sender.sendMessage("§eUse /cf admin setinv");
						return true;
					}
					if(plugin.status!=0) {
						sender.sendMessage("§9[ⒸⒻ] §cTem um CF em execução.");
						return true;
					}
					plugin.inventario=((Player)sender).getInventory().getContents();
					plugin.getConfig().set("sistema.itens.inventario", plugin.inventario);
					((Player)sender).getInventory().setContents(plugin.inventario);sdasad
					
					plugin.armadura=((Player)sender).getInventory().getArmorContents();
					plugin.getConfig().set("sistema.itens.armadura", plugin.armadura);
					((Player)sender).getInventory().setArmorContents(plugin.armadura);sadsad
					return true;
				}
				plugin.help((Player)sender);
				return true;
